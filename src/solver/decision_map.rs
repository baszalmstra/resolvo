...
/// A [`DecisionMap`] slot: the variable's assignment fused with its
/// at-most-one group membership (`NO_GROUP` when it has none), so a single
/// load answers both questions.
#[derive(Copy, Clone)]
struct Entry {
...
pub(crate) struct DecisionMap {
    /// One entry per variable: assignment fused with group membership.
    map: Vec<Entry>,
...
impl DecisionMap {
    ...
    pub fn reset(&mut self, variable_id: VariableId) {
        ...
        if entry.group == NO_GROUP {
            self.map[index].decision = DecisionAndLevel::undecided();
            return;
        }

        let group_index = entry.group as usize;
        if self.groups[group_index].chosen == Some(variable_id) {
            ...
            for &member in &members {
                let member_entry = &mut self.map[member.to_index()];
                if member_entry.decision.is_group_falsified() {
                    member_entry.decision = DecisionAndLevel::undecided();
                }
            }
            ...
            let level = self.map[chosen.to_index()].decision.level();
            self.map[index].decision = DecisionAndLevel::group_falsified(level);
        } else {
            self.map[index].decision = DecisionAndLevel::undecided();
        }
    }
    ...
    pub fn set(&mut self, variable_id: VariableId, value: bool, level: u32) {
        ...
        debug_assert!(
            !entry.decision.is_group_falsified(),
    ...
    fn choose(&mut self, group: AmoGroupId, variable_id: VariableId, level: u32) {
        ...
        for &member in &members {
            ...
            let member_entry = &mut self.map[member.to_index()];
            if member_entry.decision.value().is_none() {
                member_entry.decision = DecisionAndLevel::group_falsified(level);
            }
        }
        let group = &mut self.groups[group_index];
        group.members = members;
    ...
    pub fn group_falsified_by(&self, variable_id: VariableId) -> Option<VariableId> {
        ...
        if !entry.decision.is_group_falsified() {
            return None;
        }
        let chosen = self.groups[entry.group as usize]
            .chosen
    ...
    pub fn amo_group_of(&self, variable_id: VariableId) -> Option<AmoGroupId> {
        match self.map.get(variable_id.to_index()) {
            Some(entry) if entry.group != NO_GROUP => Some(AmoGroupId(entry.group)),
            _ => None,
        }
    }
    ...
    pub fn add_amo_member(&mut self, group: AmoGroupId, variable: VariableId) -> AmoMemberAdded {
        ...
        self.map[index].group = group.0;

        let value = self.map[index].decision.value();
        ...
        match (value, self.groups[group.to_index()].chosen) {
            ...
            (Some(true), None) => {
                let level = self.map[index].decision.level();
